...
import { chevronBack, copyOutline, homeOutline, mailOutline } from 'ionicons/icons';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, IonButton, IonContent, IonBadge, IonIcon, IonGrid, IonRow, IonCol, IonToast, CompartmentItem, BannerButton]
})
export class PackageViewPage implements OnInit {
  ...
  constructor() {
    addIcons({ chevronBack, homeOutline, mailOutline, copyOutline })
  }
  ngOnInit(): void {
  ...
  async onCopyPickupCode() {
    await Clipboard.write({
      string: this.delivery?.compartment.pickup_code
    });
  }
}
